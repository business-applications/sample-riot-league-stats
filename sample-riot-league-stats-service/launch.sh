...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "sample-riot-league-stats-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name sample-riot-league-stats-service apps/sample-riot-league-stats-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app sample-riot-league-stats-service:1.0-SNAPSHOT
    oc expose svc/sample-riot-league-stats-service
else

	echo "Launching the application locally..."
	pattern="sample-riot-league-stats-service"
	files=( $pattern )
	cd ${files[0]}
