...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "sample-riot-league-stats-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name sample-riot-league-stats-service apps/sample-riot-league-stats-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app sample-riot-league-stats-service:1.0-SNAPSHOT
		call oc expose svc/sample-riot-league-stats-service
	) else (
		echo "Launching the application locally..."
		cd sample-riot-league-stats-service
		cd target
